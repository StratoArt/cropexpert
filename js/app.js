...
const iconId=file=>String(file||'').split('/').pop().replace(/\.svg$/i,'');
const iconSvg=(file,cls='sprite-icon',style='')=>{const id=iconId(file); return `<svg class="${esc(cls)}" ${style?`style="${esc(style)}"`:''} viewBox="0 0 100 100" aria-hidden="true"><use href="assets/icons.svg#${esc(id)}"></use></svg>`};
...
function photoBlock(x){
  ...
  return `<div class="photo-card">
    <div class="photo-wrap"><img src="${esc(src)}" alt="Foto referensi ${esc(x.name)}" loading="lazy" onerror="this.closest('.photo-card').classList.add('photo-error')"><div class="photo-fallback">${iconSvg(x.icon,'sprite-icon photo-fallback-icon')}<span>Foto tidak tersedia offline</span></div></div>
...
function photoThumb(x){
  const img=(x.images||[])[0];
  if(!img) return `${iconSvg(x.icon,'sprite-icon photo-fallback-icon')}`;
  const src=img.local_path || img.remote_url;
  return `<div class="thumb-photo"><img src="${esc(src)}" alt="" loading="lazy" onerror="this.parentElement.classList.add('photo-error')"><div class="thumb-fallback">${iconSvg(x.icon,'sprite-icon photo-fallback-icon')}</div></div>`;
}
...
function renderTypes(){
  ...
  if(type==='Hama'){
    $('#typeGrid').innerHTML=source.map(x=>`<button class="type-card" data-cat="${esc(x.name)}">${iconSvg(x.icon,'sprite-icon')}<strong>${esc(x.name)}</strong><small>${esc(x.label)}</small><i>›</i></button>`).join('');
    $$('.type-card').forEach(b=>b.addEventListener('click',()=>{state.pestFilter=b.dataset.cat;showScreen('pests');renderPests()}));
  }else{
    $('#typeGrid').innerHTML=source.map(x=>`<button class="type-card" data-go-list="${esc(x.id)}">${iconSvg(x.icon,'sprite-icon')}<strong>${esc(x.name)}</strong><small>${esc(x.common)}</small><i>›</i></button>`).join('');
...
function renderCrops(){
  $('#cropGrid').innerHTML=state.opt.crops.map(x=>`<button class="crop-card" data-id="${esc(x.id)}">${iconSvg(x.icon,'sprite-icon')}<strong>${esc(x.name)}</strong><small>${esc(x.latin)}</small></button>`).join('');
...
function openOptDetail(x,type){
  ...
  $('#detail').innerHTML=`<div class="detail-backdrop" id="detailBackdrop"></div><aside class="detail-sheet">
    ...
    <div style="text-align:center;margin:16px 0 6px">${iconSvg(x.icon,'detail-sprite','width:180px;height:150px;color:#0a7548')}</div>
...
function openCropOptRef(ref){
  ...
  $('#detail').innerHTML=`<div class="detail-backdrop" id="detailBackdrop"></div><aside class="detail-sheet">
    ...
    <div style="text-align:center;margin:18px 0 8px">${iconSvg(icon,'detail-sprite','width:150px;height:125px')}</div>
